...
{
    public class SevenChan : BaseScraper, IScrapeProcessor
    {
        public SevenChan(ScrapeRepositories scrapeRepositories) : base(scrapeRepositories)
        {
        }
        ...
        public bool GetWhoStamp(IElement element, ImageDetail image)
        {
            try
            {
                var nameElement = element.GetElementsByClassName("postername");
        ...
        public bool GetResolution(IElement element, ImageDetail image)
        {
            ...
            try
            {
                ...
                var fileElement = element.GetElementsByClassName("file_size");
        ...
        private List<string> GetPageThreads(string page)
        {
            try
            {
                ...
                output.AddRange(document.GetElementsByClassName("thread")
                    .SelectMany(thread => thread.GetElementsByClassName("replylinks"))
                    .SelectMany(replyLinks => replyLinks.Children.Where(element => element.HasAttribute("href")))
